<?php
/**
 * Logs view — unified compact design.
 */
...
<div class="geo-forge-wrap">
	<div class="geo-forge-header" style="margin-bottom:14px;">
		<h1>GEO Forge <span class="geo-forge-subtitle">Logs</span></h1>
	...
	<div class="geo-forge-card">
		<div class="geo-forge-filter">
			<form method="get" class="pure-form" style="display:flex;align-items:center;gap:8px;">
			...
			<button type="button" id="geo-forge-clear-logs" class="pure-button">Clear Logs</button>
	...
	<div class="geo-forge-card">
		...
		<?php else: ?><table class="pure-table"><thead><tr><th>Time</th><th>Level</th><th>Source</th><th>Message</th><th>Req</th></tr></thead><tbody>
		<?php foreach($rows as $row): $lvl=Level::tryFrom((string)$row['level'])??Level::Debug; $ctx=is_array($row['context'])?$row['context']:[];?>
			<tr><td style="font-size:10px;white-space:nowrap;"><?php echo esc_html($row['created_at']);?></td><td><span class="geo-forge-log-badge <?php echo esc_attr($lvl->css_class());?>"><?php echo esc_html($lvl->label());?></span></td><td style="font-size:11px;"><?php echo esc_html($row['source']);?></td><td style="font-size:11px;"><?php echo esc_html($row['message']);?><?php if(!empty($ctx)):?> <details style="display:inline;"><summary style="font-size:10px;">context</summary><pre style="font-size:10px;max-width:400px;"><?php echo esc_html(wp_json_encode($ctx,JSON_PRETTY_PRINT));?></pre></details><?php endif;?></td><td style="font-size:10px;"><?php echo esc_html(substr((string)$row['request_id'],0,8));?></td></tr>
